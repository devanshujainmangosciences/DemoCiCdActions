...
import RadiationTherapy from '../../pages/clinical-details/RadiationTherapy';
import {
...
describe('RadiationTherapy reports test cases', () => {
  test('Rendering RadiationTherapy Reports page', async () => {
    ...
    const otherTest = screen.getByText('radiationTherapy');
    expect(otherTest).toBeInTheDocument();
  });
  test('RadiationTherapy data is not avaliable', async () => {
    const store = setupStore(reportsWhenNoDataIsAvaliable);
    render(
      <Provider store={store}>
        <RadiationTherapy />
  ...
  test('RadiationTherapy When Data is Avaliable', async () => {
