import {render, screen, waitFor} from '@testing-library/react';
import {beforeEach, describe, expect, test, vi} from 'vitest';
import {BrowserRouter} from 'react-router-dom';
import userEvent from '@testing-library/user-event';
import Medications from '../../pages/clinical-details/Medications';
import {
  reportsWhenNoDataIsAvaliable,
  reportsWhenDataIsAvaliable,
} from '../testAppSelector';
import {Provider} from 'react-redux';
import {setupStore} from '../../redux/store';

afterEach(() => {
  vi.clearAllMocks();
});
describe('Medication reports test cases', () => {
  test('Rendering Medication Reports page', async () => {
    const store = setupStore(reportsWhenNoDataIsAvaliable);
    render(
      <Provider store={store}>
        <Medications />
      </Provider>
    );

    const labReports = screen.getByText('medications');
    expect(labReports).toBeInTheDocument();
  });
  test('Medication data is not avaliable', async () => {
    const store = setupStore(reportsWhenNoDataIsAvaliable);
    render(
      <Provider store={store}>
        <Medications />
      </Provider>
    );
    const nodataNote = screen.getByText('noYearNote');
    expect(nodataNote).toBeInTheDocument();
  });

  test('Medication When Data is Avaliable', async () => {
    const store = setupStore(reportsWhenDataIsAvaliable);
    render(
      <Provider store={store}>
        <Medications />
      </Provider>
    );
    const cancerMedication = screen.getAllByText('Cancer Medication');
    expect(cancerMedication.length).toBe(2);
  });
  test('When Clicked on Year Tab', async () => {
    const store = setupStore(reportsWhenDataIsAvaliable);
    render(
      <Provider store={store}>
        <Medications />
      </Provider>
    );
    const handleSubmit = vi.fn();
    const comboboxes = screen.getAllByRole('combobox');
    expect(comboboxes.length).toBe(2);
    const yearComboBox = comboboxes[0];
    const monthCombobox = comboboxes[1];
    await userEvent.selectOptions(yearComboBox, '2020');
    await userEvent.selectOptions(monthCombobox, 'October');
    const filterButton = screen.getByRole('button', {
      name: /filter now/i,
    });
    await userEvent.click(filterButton);
    waitFor(() => {
      expect(handleSubmit).toBeCalled();
    });
  });
  test('When Changing the Month Tab', async () => {
    const store = setupStore(reportsWhenDataIsAvaliable);
    render(
      <Provider store={store}>
        <Medications />
      </Provider>
    );
    const handleSubmit = vi.fn();
    const monthItem = screen.getAllByRole('listitem');
    expect(monthItem.length).toBe(12);
    const januaryMonth = monthItem[11];
    await userEvent.click(januaryMonth);
    const drugName = screen.getByText('Osimertinib');
    expect(drugName).toBeInTheDocument();
    waitFor(() => {
      expect(handleSubmit).toBeCalled();
    });
  });
  test('On Procedure Change', async () => {
    const store = setupStore(reportsWhenDataIsAvaliable);
    render(
      <Provider store={store}>
        <Medications />
      </Provider>
    );
    const handleSubmit = vi.fn();
    const monthItem = screen.getAllByRole('listitem');
    expect(monthItem.length).toBe(12);
    const januaryMonth = monthItem[11];
    await userEvent.click(januaryMonth);
    const noncancerMedication = screen.getByRole('tab', {
      name: 'Non-Cancer Medication (1)',
    });
    expect(noncancerMedication).toBeInTheDocument();
    await userEvent.click(noncancerMedication);
    waitFor(() => {
      expect(handleSubmit).toBeCalled();
      const drugName = screen.getByText('Dexamethasone');
      expect(drugName).toBeInTheDocument();
    });
  });
});
